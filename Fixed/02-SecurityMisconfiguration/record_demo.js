...
async function run() {
  ...
  console.log('Navigating to http://localhost:5014...');
  await page.goto('http://localhost:5014');
  await page.waitForTimeout(2000);

  console.log('Step 1: Testing sanitized error problem details...');
  await page.click('#btn-error');
  await page.waitForTimeout(3000);

  console.log('Step 2: Inspecting security response headers...');
  ...
  if (videoPath && fs.existsSync(videoPath)) {
    const finalDest = path.join(recordingsDir, 'owasp-a05-security-misconfiguration-fixed.webm');
