...
async function run() {
  ...
  console.log('Navigating to http://localhost:5009...');
  await page.goto('http://localhost:5009');
  await page.waitForTimeout(2000);

  console.log('Step 1: Executing SQL Injection attack...');
  await page.click('#btn-sql');
  await page.waitForTimeout(3000);

  console.log('Step 2: Executing Command Injection attack...');
  ...
  if (videoPath && fs.existsSync(videoPath)) {
    const finalDest = path.join(recordingsDir, 'owasp-a03-injection.webm');
